...
def get_args():
    ...
    arg("-d", "--dataset", help="dataset", default="pv", choices=["pv", "landcoverai", "uavid", "building", "inria"])
...
def main():
    ...
    for img_path in img_paths:
        ...
        if args.dataset == 'landcoverai':
            ...
            output_mask = inria_mask(output_mask)
        else:
            output_mask = output_mask



        output_img = cv2.imread(img_path, cv2.IMREAD_COLOR)
        output_map = np.zeros(shape=(output_height, 2 * output_width, 3), dtype=np.uint8)
        output_map[0:output_height, 0:output_width] = output_img
        output_map[0:output_height, output_width:2*output_width] = output_mask
        cv2.imwrite(os.path.join(args.output_path, img_name), output_map)
