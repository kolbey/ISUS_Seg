...
weights_name = "BA-HV"
weights_path = "model_weights/loveda2/{}".format(weights_name)  # do not change
test_weights_name = "BA-HV"  # if save_top_k=3, there are v1,v2 model weights, i.e.xxx-v1, xxx-v2
log_name = 'loveda2/{}'.format(weights_name)  # do not change
...
save_last = False  # save the last model weight, e.g. test_weights_name='last'
check_val_every_n_epoch = 1  # run validation every n epoch
gpus = [3]  # gpu ids, 0, 1, 2.., more setting can refer to pytorch_lightning
...
train_dataset = LoveDATrainDataset(transform=train_aug, data_root='../DATASET/Original_LoveDA/train')
