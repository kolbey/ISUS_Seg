...
class Supervision_Train(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.config.contrastive:
            pre_mask = F.interpolate(prediction[0], size=(mask.shape[1], mask.shape[2]), mode='bilinear', align_corners=True)
            pre_mask = nn.Softmax(dim=1)(pre_mask)
        else:
    ...
    def validation_epoch_end(self, outputs):
        if 'vaihingen' in self.config.log_name:
            ...
            mIoU = np.nanmean(self.metrics_val.Intersection_over_Union()[:-1])
            F1 = np.nanmean(self.metrics_val.F1()[:-1])
        elif 'flair' in self.config.log_name:
            mIoU = np.nanmean(self.metrics_val.Intersection_over_Union()[:-1])
            F1 = np.nanmean(self.metrics_val.F1()[:-1])
        else:
...
def main():
    ...
    checkpoint_callback_ckpt = ModelCheckpoint(save_top_k=config.save_top_k, monitor=config.monitor,
                                          save_last=config.save_last, mode=config.monitor_mode,
                                          dirpath=config.weights_path,
                                          filename=config.weights_name)

    logger = TensorBoardLogger('tensorboard_logs', name=config.log_name)
    ...
    trainer = pl.Trainer(devices=config.gpus, max_epochs=config.max_epoch, accelerator='gpu',
                         ...
                         callbacks=[checkpoint_callback_ckpt],
