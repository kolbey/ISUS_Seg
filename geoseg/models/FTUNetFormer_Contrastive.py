...
class FTUNetFormer(nn.Module):
    ...
    def __init__(self,
        ...
        self.projhead = ProjectionHead(dim_in=embed_dim*8)
        self.contrastive = contrastive

    def forward(self, x):
        h, w = x.size()[-2:]
        res1, res2, res3, res4 = self.backbone(x)
        x = self.decoder(res1, res2, res3, res4, h//8, w//8)
        if self.contrastive:
            embending_res4 = F.interpolate(res4, size=(h//8, w//8), mode="bilinear", align_corners=True)
            embending = self.projhead(embending_res4)
            return [x, embending]
        else:
            x = F.interpolate(x, size=(h, w), mode="bilinear", align_corners=True)
            return x
...
def ft_unetformer(contrastive=False, pretrained=False, num_classes=6,
                  freeze_stages=-1, decoder_channels=256,
                  weight_path=None):
    model = FTUNetFormer(contrastive=contrastive,
