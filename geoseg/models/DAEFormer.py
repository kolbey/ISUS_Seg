# kolbey
# 2023-02-02
# https://arxiv.org/pdf/2212.13504.pdf
...
from geoseg.models.SegFormer import *
...
class DAEFormer(nn.Module):
    def __init__(self, num_classes=9, decoder_feat_size=7, head_count=1, token_mlp_mode="mix_skip"):
        super().__init__()
        ...
        # Decoder
        self.d_base_feat_size = decoder_feat_size  # 32 for 1024 input size, 16 for 512 input size, and 7 for 224
        in_out_chan = [
            [64, 128, 128, 128, 160],
            [320, 320, 320, 320, 256],
            [512, 512, 512, 512, 512],
        ...
        self.decoder_2 = MyDecoderLayer(
            (self.d_base_feat_size * 2, self.d_base_feat_size * 2),
            in_out_chan[2],
            head_count,
            token_mlp_mode,
            n_class=num_classes,
        ...
        self.decoder_1 = MyDecoderLayer(
            (self.d_base_feat_size * 4, self.d_base_feat_size * 4),
            in_out_chan[1],
            head_count,
            token_mlp_mode,
            n_class=num_classes,
        ...
        self.decoder_0 = MyDecoderLayer(
            (self.d_base_feat_size * 8, self.d_base_feat_size * 8),
            in_out_chan[0],
            head_count,
            token_mlp_mode,
            n_class=num_classes,
    ...
    def forward(self, x):
        ...
        # if x.shape(4,3,512,512), output_enc[0].shape(4,128,128,128)
        # if x.shape(4,3,512,512), output_enc[1].shape(4,320,64,64)
        # if x.shape(4,3,512,512), output_enc[2].shape(4,512,32,32)
...
if __name__ == '__main__':
    input = torch.randn(4,3,512,512).cuda(7)
    model = DAEFormer(num_classes=13, decoder_feat_size=16).cuda(7)

    output = model(input)

    # output.shape(4,13,512,512)
    print(output.shape)