...
class SegFormer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        decoder_outs = self.decode_head(encoder_outs)

        output = F.interpolate(decoder_outs, size=(height, width), mode='bilinear', align_corners=True) # 上采样至原图大小
        return output

if __name__ == '__main__':
    input = torch.randn(4,3,512,512).cuda(0)
    model = SegFormer(model_name='B5', num_classes=13, image_size=512).cuda(0)
