# 手动 K 折交叉验证提升模型泛化能力，有时间可以改成自动 K 折
...
class FLAIRKTrainDataset(Dataset):
    def __init__(self, data_root='../flair',
        ...
        self.data_ids = self.load_data(self.data_root)
    ...
    def load_data(self, data_root):
        ...
        def _gather_data(path_folders, test_set: bool) -> dict:
        ...
        dict_trainval = _gather_data(trainval_domains,test_set=False)

        kfold = 5
        split_idx = len(dict_trainval['IMG']) // kfold

        dict_train = {'IMG':[], 'MSK':[]}
        dict_val = {'IMG':[], 'MSK':[]}
        dict_train['IMG'] = dict_trainval['IMG'][:(4*split_idx)]
        dict_train['MSK'] = dict_trainval['MSK'][:(4*split_idx)]

        dict_val['IMG'] = dict_trainval['IMG'][(4*split_idx):]
        dict_val['MSK'] = dict_trainval['MSK'][(4*split_idx):]
...
class FLAIRKTestDataset(Dataset):
    def __init__(self, data_root='../flair',
                 path_metadata='../flair/flair-one_metadata.json',
    ...
    def load_data(self, data_root):
        ...
        def _gather_data(path_folders) -> dict:
        ...
        dict_test = _gather_data(test_domains)
