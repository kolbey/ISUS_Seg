...
class EdgeLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        loss1 = self.main_loss(logits, targets)
        loss2 = self.compute_edge_loss(logits, targets) * self.edge_factor
        loss = loss1 + loss2
        return loss
...
class UFLossV1(nn.Module):
    ...
    def __init__(self, ignore_index=255):
        ...
        self.main_loss = JointLoss(SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index),
                                   DiceLoss(smooth=0.05, ignore_index=ignore_index), 1.0, 1.0)
        # self.aux_loss = SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index)
        self.aux_loss = EdgeLoss(ignore_index=ignore_index,edge_factor=0.4)
...
class UnetPlusPlusLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        if self.training and len(logits) == 4:
            [out_put1, out_put2, out_put3, out_put4] = logits
            # logit_main, logit_aux = logits
            loss = self.main_loss(out_put4, labels) + 0.4 * self.aux_loss(out_put3, labels) + 0.4 * self.aux_loss(out_put2, labels) + 0.4 * self.aux_loss(out_put1, labels)
        else:
            loss = self.main_loss(logits, labels)
